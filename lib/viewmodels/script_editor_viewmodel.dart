...
class ScriptEditorViewModel extends ChangeNotifier {
  final _logger = LoggerConfig.getLogger('ScriptEditorViewModel');
  ...
  ScriptEditorViewModel({Map<String, dynamic>? scriptData}) {
    ...
    if (scriptData != null) {
      ...
      _logger.info('Loaded script data: $scriptData');
      _logger.info('Initialized title: $_scriptName');
      _logger.info('Initialized combi name: $_combiName');
      _logger.info('Initialized boke name: $_bokeName');
      _logger.info('Initialized tsukkomi name: $_tsukkomiName');

      // vvox speaker_idの設定
      if (scriptData['voices']?.isNotEmpty == true) {
        final voices = scriptData['voices'] as List<dynamic>;
        final bokeVoice = voices.firstWhere((v) => v['characterType'] == 'left',
            orElse: () => {'speaker_id': 1});
      ...
      _logger
          .info('Initialized state - Title: $_scriptName, Combi: $_combiName');
      notifyListeners();
    }
  }
  ...
  Future<void> _initTts() async {
    try {
      ...
      _logger.severe('TTS initialization failed', e);
  ...
  void addScriptLine(String text) {
    ...
    _logger.info("_selectedSpeed:$_selectedSpeed ");
    _logger.info("_selectedVolume:$_selectedVolume ");
    _logger.info("_selectedPitch:$_selectedPitch ");
    _scriptLines.add(
      ScriptLine(
  ...
  Future<void> playScriptLine(ScriptLine line) async {
    try {
      ...
      _logger.info("line.speed:${line.speed}");
  ...
  Future<void> createAudioFile() async {
    ...
    try {
      ...
      _logger.info('Voicevox API リクエスト送信: ${Uri.parse('$serverUrl/concat')}');
      ...
      if (response.statusCode == 200) {
        _logger.info('音声結合API呼び出しが成功しました');
        _logger.info('レスポンス: ${response.body}');

        // responseDataを使用した処理を記述
      } else {
        _logger.severe('APIエラー: ステータスコード ${response.statusCode}', response.body,
            StackTrace.current);
        throw Exception('Failed to call Voicevox API: ${response.statusCode}');
      }
    } catch (e, stackTrace) {
      _logger.severe('Voicevox API 呼び出し中にエラーが発生しました', e, stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> saveManzaiData(BuildContext context) async {
    try {
      final manzaiData = {
        ...
        "voices": _scriptLines
            .map((line) => {
                  "text": line.text,
                  "speaker_id":
                      line.characterType == 'ボケ' ? _bokeVoice : _tsukkomiVoice,
                  "characterType":
                      line.characterType == 'ボケ' ? "left" : "right",
                  "speed_scale": line.speed,
                  "volume_scale": line.volume,
                  "pitch_scale": line.pitch,
                  "intonation_scale": line.intonation,
      ...
      final response = await http.post(
        Uri.parse('http://localhost:8000/scripts/save_manzai_script'),
        headers: {'Content-Type': 'application/json'},
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('漫才データを保存しました')));
      } else {
        throw Exception('サーバーエラー: ${response.statusCode}');
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('保存に失敗しました: $e'), backgroundColor: Colors.red));
    }
  }
  ...
  Future<void> exportMarkdown() async {
    ...
    for (var line in _scriptLines) {
      // キャラクター名と台詞を追加
      mdScript.writeln(
          '* ${line.characterType == "ボケ" ? _bokeName : _tsukkomiName}: ${line.text}');
      // パラメータ情報を追加
      mdScript.writeln(
          '  * スピード:${line.speed}x, 間:${line.timing}秒, 声量:${line.volume}, 声高:${line.pitch},抑揚:${line.intonation}');
    }
    // 2. ダウンロード用の設定
  ...
  Future<void> showEditDialog(
    ...
    final timingController =
        TextEditingController(text: line.timing.toString());
    final speedController = TextEditingController(text: line.speed.toString());
    final volumeController =
        TextEditingController(text: line.volume.toString());
    final pitchController = TextEditingController(text: line.pitch.toString());
    final intonationController =
        TextEditingController(text: line.intonation.toString());
  ...
  Future<void> generateVideo() async {
    ...
    try {
      ...
      final List<html.Blob> chunks =
          []; // Changed from List<html.File> to List<html.Blob>
      ...
      recorder.addEventListener('dataavailable', (html.Event event) {
        if (event is html.BlobEvent && event.data != null) {
          chunks.add(
              event.data!); // Add Blob directly instead of converting to File
        }
      });
  ...
  Future<void> _downloadVideo(List<html.Blob> chunks) async {
    ...
    html.AnchorElement()
      ..href = url
      ..download =
          '${_scriptName}_${DateTime.now().millisecondsSinceEpoch}.webm'
      ..click();
  ...
  Future<void> _initializeAndStartBGM() async {
    if (_selectedMusic != null) {
      ...
      try {
        ...
        _logger.severe('BGM initialization error', e);
...
// === 変数からvoicevox用に台本全体を作成 ===
  Map<String, dynamic> createVoicevoxScript() {
    // voicesリストの作成
    final List<Map<String, dynamic>> voices = _scriptLines
        .map((line) => {
              "text": line.text,
              "speaker_id":
                  line.characterType == 'ボケ' ? _bokeVoice : _tsukkomiVoice,
  ...
  Future<void> _prefetchAudio(int index) async {
    ...
    try {
      ...
      _logger.warning('Prefetch error for index $index', e);
    } finally {
  ...
  Future<void> _startAnimation() async {
    ...
    try {
      ...
      _dialogueProgressController
          ?.add(_dialogueCurrentIndex / _scriptLines.length);
      ...
      if (!_isDisposed) {
        ...
        _logger.severe('Animation error', e);
      }
    }
  }

  Future<void> _playAnimationAudio(
      ScriptLine line, BytesSource audioSource) async {
    final completer = Completer<void>();
